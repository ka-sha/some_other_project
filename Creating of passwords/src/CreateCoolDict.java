...
public class CreateCoolDict {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 2; i < symbols.length; i++) {
            ...
            if (symbols[i] == '\n' && i + 2 < symbols.length) {
                if (!chain.containsKey("\n"))
                    chain.put("\n", new ArrayList<>());
                String str = "" + (char) symbols[i + 1] + (char) symbols[i + 2];
        ...
        while (count < 5000000) {
            ...
            curStr = finalString.substring(finalString.length() - 2);
            if (curStr.charAt(1) == '\n')
                curStr = "\n";
        }
